...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int x = 0; x <= _width - 1; x += 1) {
            for (int y = 0; y <= _height - 1; y += 1) {
                int myint = _solution[x][y];
                _solnNumToPlace[myint] = Place.pl(x, y);
            }
        }
        ...
        for (Sq[] col: _board) {
            for (Sq sq: col) {
                ...
                for (int x = 0; x <= _width - 1; x += 1) {
                    for (int y = 0; y <= _height - 1; y += 1) {
                        int dir = Place.dirOf(x, y, sq.x, sq.y);
                        if (dir > 0) {
                            sq._predecessors.add(Place.pl(x, y));
                        }
                    }
                }
    ...
    Model(Model model) {
        ...
        for (int x = 0; x <= width() - 1; x += 1) {
            for (int y = 0; y <= height() - 1; y += 1) {
                Sq origsq = model._board[x][y];
                _board[x][y] = new Sq(origsq);
    ...
    boolean autoconnect() {
        for (Sq[] col: _board) {
            for (Sq sq: col) {
                if (sq.sequenceNum() != 0) {
                    sq.connect(get(_solnNumToPlace[sq.sequenceNum() + 1]));
                    return true;
                }
            }
        }
    ...
    void solve() {
        for (int h = 1; h <= _width * _height; h += 1) {
            Place myplace = solnNumToPlace(h);
            _board[myplace.x][myplace.y]._sequenceNum = h;
        }
        _unconnected = 0;
    }
    ...
    private int arrowDirection(int x, int y) {
        ...
        int totalsquares = _height * _width;
        Place nextplace = _solnNumToPlace[seq0 + 1];
        if (0 < (seq0 + 1) && (seq0 + 1) <= totalsquares) {
            return Place.dirOf(x, y, nextplace.x, nextplace.y);
        } else {
            return 0;
        }
    }
    ...
    final class Sq {
         ...
         * Attended project party where GSI explained connect logic */
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() != 0) {
                ...
                while (current != null && current._successor != null) {
                ...
                while (current != null && current._predecessor != null) {
            ...
            for (Place place: this._successors) {
                Sq curr = get(place);
                curr._head = this.head();
            }
            return true;
        }
